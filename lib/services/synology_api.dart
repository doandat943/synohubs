...
class SynologyApi {
  ...
  Future<Map<String, dynamic>> login(String account, String passwd) async {
    // Resolve domain → IP via DoH if system DNS fails
    await _ensureHostResolved();
    final resp = await _get('auth.cgi', {
      'api': 'SYNO.API.Auth',
      'version': '6',
      'method': 'login',
      'account': account,
  ...
  /// Check if the given account is an admin by trying to login with 'SynoHub'
  /// session. Only admin users can login with this session name — non-admin
  /// users get error 402 (permission denied). The main FileStation SID is
  /// preserved; the temporary session is discarded after the check.
  Future<bool> checkAdmin(String account, String passwd) async {
    final savedSid = _sid;
    try {
      final loginResp = await _get('auth.cgi', {
        'api': 'SYNO.API.Auth',
        'version': '6',
        'method': 'login',
        'account': account,
      ...
      if (loginResp['success'] != true) return false;

      // Login succeeded → user is admin. Logout the temp session.
      final tempSid = loginResp['data']?['sid'] as String?;
      if (tempSid != null) {
        _sid = tempSid;
        try {
          await _get('auth.cgi', {
      ...
      _sid = savedSid; // restore FileStation SID
    }
  }
